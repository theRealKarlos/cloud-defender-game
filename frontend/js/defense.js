...
class Defense extends Entity {
    ...
    updateEffects(deltaTime) {
        if (this.firingEffectTimer > 0) {
            this.firingEffectTimer = Math.max(0, this.firingEffectTimer - deltaTime);
        }
    }
    ...
    renderDefenseIcon(ctx) {
        ...
        ctx.strokeText(this.defenseIcon, this.x + this.width / 2, this.y + this.height / 2);
        ctx.fillText(this.defenseIcon, this.x + this.width / 2, this.y + this.height / 2);
    ...
    renderStatusIndicators(ctx) {
        ...
        if (this.currentCooldown > 0) {
            ...
            ctx.lineWidth = 3;
            ctx.beginPath();
            ctx.arc(centerX, centerY, this.width / 2 + 4, -Math.PI / 2, -Math.PI / 2 + angle);
            ctx.stroke();
            ctx.restore();
        }
    }
    ...
    renderChargingEffect(ctx) {
        ...
        ctx.beginPath();
        ctx.arc(centerX, centerY, this.width / 2 + 6, -Math.PI / 2, -Math.PI / 2 + angle);
        ctx.stroke();
        
        // Draw charging particles
        for (let i = 0; i < 4; i++) {
            const particleAngle = (this.age * 5 + i * Math.PI / 2) % (Math.PI * 2);
    ...
    renderFiringEffect(ctx) {
        const effectPercent = 1 - (this.firingEffectTimer / this.firingEffectDuration);
        const centerX = this.x + this.width / 2;
        const centerY = this.y + this.height / 2;
...
class DefenseProjectile extends Entity {
    constructor(defenseType, startX, startY, targetX, targetY, damage, speed, sourceDefense) {
