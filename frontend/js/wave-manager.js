...
class WaveManager {
  ...
  startWave(waveNumber = null) {
    ...
    console.log(
      `Starting Wave ${this.currentWave}: ${config.missileCount} missiles, ${config.spawnInterval}s interval`
    );
  ...
  spawnMissile(config) {
    ...
    console.log(
      `Wave ${config.waveNumber}: Missile speed ${originalSpeed} → ${missile.speed.toFixed(1)} (${(missileSpeedMultiplier * 100).toFixed(1)}%)`
    );
    ...
    console.log(
      `Spawned ${missileType} missile (${this.missilesSpawnedInWave}/${this.missilesInCurrentWave})`
    );
  }
  ...
  applyBossWaveEvent() {
    ...
    activeMissiles.forEach((missile) => {
      if (Math.random() < 0.3) {
        // 30% chance to become "boss" missile
        missile.scale = 1.5;
        missile.damage *= 2;
  ...
  applySpeedBurstEvent() {
    ...
    if (Math.random() < 0.1) {
      // 10% chance per frame during speed burst
  ...
  applyMultiSpawnEvent(config) {
    ...
    if (
      Math.random() < 0.05 &&
      this.missilesSpawnedInWave < this.missilesInCurrentWave - 1
    ) {
      this.spawnMissile(config);
    }
  }
