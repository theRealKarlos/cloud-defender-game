...
class ApiService {
  constructor() {
    // Configure API base URL - automatically detect or use fallback
   ...
   * Always uses the deployed API Gateway URL for both local and production
   */
  getApiBaseUrl() {
    // Check if we're running locally and need CORS proxy
    const isLocalhost = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
    
    // Always use the configured API URL from config.js
    if (window.API_CONFIG && window.API_CONFIG.baseUrl) {
      return window.API_CONFIG.baseUrl;
    }

    // Fallback URL - this should be updated by the deployment script
    // Both local development and production use the same deployed API
    return "https://n8zszsigg3.execute-api.eu-west-2.amazonaws.com/dev";
  }

  /**
   * Check if we need to use alternative request methods due to CORS
   */
  needsCorsWorkaround() {
    const isLocalhost = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
  ...
  async submitScore(scoreData, validationData = null) {
    try {
      ...
      console.log("Submitting score with security validation:", {
        ...
        requestUrl: `${this.API_BASE_URL}/api/scores`,
      ...
      if (error.name === "AbortError") {
        ...
        if (isLocalhost && isHttps) {
          ...
          corsError.solutions = {
            deployCommand: 'cd infra && terraform apply -target="module.lambda_function" -auto-approve',
            chromeCommand: 'chrome.exe --user-data-dir="C:/temp/chrome-cors" --disable-web-security --disable-features=VizDisplayCompositor',
