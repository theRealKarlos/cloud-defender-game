...
class GameConditions {
  ...
  triggerVictory(victoryType) {
    ...
    switch (victoryType) {
      case 'perfect':
        ...
        victoryMessage =
          'Perfect Victory! All infrastructure defended successfully!';
        break;
      case 'pyrrhic':
        scoreBonus = this.currentScore * 0.1; // 10% bonus for pyrrhic victory
        victoryMessage =
          'Victory achieved, but at great cost. Infrastructure was compromised.';
        break;
    }
    ...
    console.log(
      `Victory achieved: ${victoryType}, Final Score: ${this.currentScore}`
    );
  }
  ...
  triggerDefeat(defeatReason) {
    ...
    switch (defeatReason) {
      case 'targets_destroyed':
        defeatMessage =
          'Game Over! All AWS infrastructure has been compromised.';
        break;
  ...
  updateScore(deltaTime) {
    ...
    this.currentScore += Math.floor(
      baseScorePerSecond * deltaTime * this.scoreMultiplier
    );
  ...
  onTargetDestroyed(target, _missile) {
    ...
    console.log(
      `Target ${target.type} destroyed! Lives remaining: ${this.currentLives}`
    );
  ...
  onWaveCompleted(waveNumber) {
    ...
    console.log(
      `Wave ${waveNumber} completed: +${Math.floor(waveBonus)} points`
    );
