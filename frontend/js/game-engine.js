...
class GameEngine {
    ...
    setupWaveCallbacks() {
        this.waveManager.setOnWaveStartCallback((waveNumber, config) => {
            console.log(
                `Wave ${waveNumber} started with ${config.missileCount} missiles`
            );
    ...
    startGame() {
        if (
            this.stateManager.isState(GameState.MENU) ||
      this.stateManager.isState(GameState.PAUSED)
        ) {
    ...
    gameOver() {
        ...
        const validationResult = this.gameSecurity.onGameOver(
            this.gameStats.score,
            this.gameStats.wave
        );
    ...
    processCollisions(collisions) {
        collisions.forEach(([entity1, entity2]) => {
            ...
            if (
                (entity1.collisionLayer === 'missiles' &&
          entity2.collisionLayer === 'targets') ||
        (entity1.collisionLayer === 'targets' &&
          entity2.collisionLayer === 'missiles')
            ) {
                const missile =
          entity1.collisionLayer === 'missiles' ? entity1 : entity2;
            ...
            if (
                (entity1.collisionLayer === 'missiles' &&
          entity2.collisionLayer === 'defences') ||
        (entity1.collisionLayer === 'defences' &&
          entity2.collisionLayer === 'missiles')
            ) {
                const missile =
          entity1.collisionLayer === 'missiles' ? entity1 : entity2;
                const _defence =
          entity1.collisionLayer === 'defences' ? entity1 : entity2;
    ...
    // Check countermeasure limit (max 4 on screen at once)
        const activeBombs =
      this.entityManager.getEntitiesByLayer('countermeasures');
    ...
    // Get all active bombs
        ...
        bombs.forEach((bomb) => {
            if (bomb.isExploding && bomb.explosionRadius > 0) {
                ...
                const destroyedMissiles = bomb.getMissilesInExplosionRadius(
                    this.entityManager
                );
