...
class SpatialGrid {
  ...
  updateEntity(entity) {
    ...
    if (
      Math.abs(bounds.left) > maxCoord ||
      Math.abs(bounds.right) > maxCoord ||
      Math.abs(bounds.top) > maxCoord ||
      Math.abs(bounds.bottom) > maxCoord
    ) {
      console.warn(
        'Entity has invalid coordinates, skipping spatial grid update'
      );
      return;
    }
    ...
    if (
      ...
      console.warn(
        'Entity has invalid grid coordinates, skipping spatial grid update',
