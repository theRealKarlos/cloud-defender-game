...
class Missile extends Entity {
    ...
    static getDimensionsByType(type) {
        const dimensionMap = {
            'cost-spike': { width: 16, height: 24 },
            'data-breach': { width: 20, height: 20 },
            'latency-ghost': { width: 14, height: 18 },
            'policy-violator': { width: 18, height: 22 }
        };
        return dimensionMap[type] || { width: 16, height: 20 };
    }
    ...
    initializeMovement() {
        ...
        if (this.movementType === 'erratic') {
            ...
            this.velocityX += (Math.random() - 0.5) * this.speed * 0.3;
            this.velocityY += (Math.random() - 0.5) * this.speed * 0.3;
        }
    }
    ...
    updateSeekingMovement(deltaTime, behaviour) {
        ...
        if (distance > 0) {
            ...
            this.velocityX = this.velocityX * (1 - seekingStrength) + 
                           targetDirectionX * this.speed * seekingStrength;
            this.velocityY = this.velocityY * (1 - seekingStrength) + 
                           targetDirectionY * this.speed * seekingStrength;
        }
    }
    ...
    updateErraticMovement(deltaTime, behaviour) {
        ...
        this.velocityX += Math.cos(randomAngle) * this.speed * changeStrength;
        this.velocityY += Math.sin(randomAngle) * this.speed * changeStrength;
    ...
    updateSlowMovement(deltaTime, behaviour) {
        // Steady movement with slight course corrections
        if (Math.random() < 0.05) { // 5% chance per frame to adjust course
            const dx = this.targetX - this.x;
            const dy = this.targetY - this.y;
            const distance = Math.sqrt(dx * dx + dy * dy);
            ...
            if (distance > 0) {
                ...
                this.velocityX += (dx / distance) * this.speed * correctionStrength;
                this.velocityY += (dy / distance) * this.speed * correctionStrength;
            }
        }
    }
    ...
    renderThreatIcon(ctx) {
        ...
        // Position text in center of missile
        ...
        ctx.strokeText(this.threatIcon, this.x + this.width / 2, this.y + this.height / 2);
        ctx.fillText(this.threatIcon, this.x + this.width / 2, this.y + this.height / 2);
    ...
    renderDataBreachEffect(ctx) {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            ctx.lineTo(centerX + Math.cos(angle) * length, 
    ...
    static createRandomMissile(startX, startY, targetX, targetY) {
        const types = ['cost-spike', 'data-breach', 'latency-ghost', 'policy-violator'];
